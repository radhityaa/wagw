...
class ApiController extends Controller
{
    ...
    public function createDevice(Request $request)
    {
        ...
        if (!$request->has('phone') || !$request->has('api_key')) {
            return response()->json(['status' => false,'msg' => 'Wrong parameters!', ],Response::HTTP_BAD_REQUEST);
        } 

        $device = Device::whereBody($request->phone)->first();
        if ($device) {
            return response()->json(['status' => false,'msg' => 'Device already', ],Response::HTTP_BAD_REQUEST);
        }
        ...
        try {
            ...
            return response()->json(['status' => true, 'msg' => 'Device Has been created', 'data' => $device]);
        } catch (\Throwable $th) {
